...
class test_main:
    __plugin_path = "/www/server/panel/plugin/test/"
    __config = None
    ...
    #示例未登录面板的情况下访问get_logs方法： /test/get_logs.json  或 /test/get_logs.html (使用模板)
    ...
    #访问/test/index.html时调用的默认方法，需要在templates中有index.html，否则无法正确响应模板
    def index(self,args):
        return self.get_logs(args)


    #获取面板日志列表
    #传统方式访问get_logs方法：/plugin?action=a&name=test&s=get_logs
    #使用动态路由模板输出： /test/get_logs.html
    #使用动态路由输出JSON： /test/get_logs.json
